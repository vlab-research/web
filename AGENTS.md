...
| **Search Console** | **The next SEO step, and the only open piece of the 2026-09-23 analytics work.** Nandan adds a **Domain** property for `vlab.digital` in Search Console (it needs his Google login) and hands over the `google-site-verification=…` string. Then: add it as a TXT record on the apex in **Netlify DNS** (`netlify api createDnsRecord`, zone `624a5c409559c35e79eedc19` — the CLI here is logged in), wait for it to resolve, press Verify. No script, no cookie, no page change. `notes/ws-seo.md` §5.5 is why DNS and not a meta tag |
