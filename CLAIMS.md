...
| C-058 | Instagram as a survey channel | **Not published** | The outbound translator exists but the client is a stub returning a not-implemented error (`fly/message-worker/stub_clients.go`, `translator_instagram.go`), and there is **no inbound path at all**. This is early work, not near-shipping, so the forward rule does not reach it. **The docs asserted Instagram support in two places. Both removed 2026-08-30**, when D-008 moved that documentation onto this domain and publishing them became our problem rather than a note about somebody else's page: `fly/reference/questions.md` claimed one media URL 'works on WhatsApp, Messenger and Instagram' and described Instagram caption behaviour. `fly/reference/channels.md` is the corrected page — Instagram is listed **Not available** and readers are told to treat it as unsupported. **Still `WITHHELD`: nothing on any surface may say Instagram is a channel** | `WITHHELD` | 2026-08-21 |
